...
const Jeu = ({ questionTabCap, questionsTab, setCount, count }) => {
  ...
  useEffect(() => {
    setReponseCorrecteDrap(
      questionDrapeau.find((objet) => objet.bonnereponse === true)
    );
  }, [click]);

  useEffect(() => {
    setReponseCorrecteCap(
      questionCapitale.find((objet) => objet.bonnereponse === true)
    );
  }, [click]);
  ...
  const handleClick = function () {
    if (!questionIsDrapeau) {
      setarrIteration(arrIteration + 1);
    }
    setCount((c) => c + 1);
              ...
              <>
                ...
                {click && !isGoodAnswer && (
                  <>
                    ...
                    <p className="Pays">
                      {clickedAnswer} au lieu de {reponseCorrecteDrap.pays}
                    </p>
                  </>
                )}
                ...
                <button
                  className={`boutonSuivantJeu ${click ? "visible" : "hidden"}`}
                  onClick={handleClick}
                >
                  Suivant
                </button>
